...
exports.createOrder = async (req, res) => {
    ...
    try {
        const { userId, restaurantId, items, total, paymentInfo } =
            req.body;
        // Validación básica
        ...
        const orderResult = await client.query(
            ...
            [userId, restaurantId, total, JSON.stringify(items),
                'pending', new Date()]
        );
        ...
        const paymentResult = await
            paymentService.processPayment(paymentInfo, total);
        ...
        await notificationService.sendOrderConfirmation(userId,
            orderResult.rows[0]);
        res.json({
            success: true,
